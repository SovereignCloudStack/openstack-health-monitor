...
if test "$RPRE" == "${RPRE%_}"; then
    echo "Need trailing _ for prefix RPRE"
    exit 1
fi
...
while test -n "$1"; do
    case $1 in
        ...
        "-s")
            SENDSTATS=1
            if test -n "$2" -a "$2" != "CLEANUP" -a "$2" != "DEPLOY" -a "${2:0:1}" != "-"; then
                SENDSTATHR="$2"
                shift
            fi
            ;;
        ...
        "-D")
            BOOTALLATONCE=1
            BOOTFROMIMAGE=1
            unset MANUALPORTSETUP
            ;;
        "-e")
            if test -z "$EMAIL"; then EMAIL="$2"; else EMAIL2="$2"; fi
            shift
            ;;
        "-m")
            if test -z "$SMNID"; then SMNID="$2"; else SMNID2="$2"; fi
            shift
            ;;
        "-q") NOALARM=1 ;;
        "-a")
        ...
        "-LP")
            LOADBALANCER=1
            TCP_LB=1
            LB_PROVIDER="--provider $2"
            shift
            ;;
        "-LO")
            LOADBALANCER=1
            TCP_LB=1
            LB_PROVIDER="--provider ovn"
            ;;
        "-b") BCBENCH=1 ;;
        "-B") IPERF=1 ;;
        "-t") let TIMEOUTFACT+=1 ;;
        ...
        "CLEANUP") break ;;
        "CONNTEST")
            if test "$MAXITER" == "-9999"; then MAXITER=1; fi
            break
            ;;
        *)
...
sendrecoveryalarm()
{
    if test $VMERRORS -gt 0 -o $WAITERRORS -gt 0 -o $APIERRORS -gt 0 -o $APITIMEOUTS -gt 0 -o $THISRUNTIME -gt $MAXCYC; then
        LASTERRITER=$loop
        return
    fi
...
translate()
{
    ...
    OSTACKCMD=("$@")
    if test "$1" == "myopenstack" -a -z "$OPENSTACKTOKEN"; then
        shift
        OSTACKCMD=("openstack" "$@")
        return 0
    fi
    if test "$1" == "openstack" -a -z "$OPENSTACKTOKEN"; then
        shift
        OSTACKCMD=("openstack" "$@")
        return 0
    fi
    if test -z "$EP"; then
        if test "$1" != "openstack"; then echo "No translation for $@" 1>&2; fi
        return 0
    fi
    if test -z "$OPENSTACKCLIENT" -o "$1" == "openstack" -o "$1" == "myopenstack"; then return 0; fi
    if test -n "$LOGFILE"; then echo "#DEBUG: $@" >> $LOGFILE; fi
    #echo "#DEBUG: $@" 1>&2
    if test -z "$DEFCMD"; then
...
ostackcmd_search()
{
    ...
    if test "$TIMEOUT" = "0"; then
        ...
        RESP=$(
            ${OSTACKCMD[@]} 2>&1 &
            TPID=$!
            killin $TPID $TIMEOUT > /dev/null 2>&1 &
            KPID=$!
            wait $TPID
            RC=$?
            kill $KPID
    ...
    if test "$RC" != "0"; then
        echo "$TIM $RC"
        echo -e "${YELLOW}ERROR: ${OSTACKCMD[@]} => $RC $RESP$NORM" 1>&2
        return $RC
    fi
    if test -z "$ID"; then
        echo "$TIM $RC"
        echo -e "${YELLOW}ERROR: ${OSTACKCMD[@]} => $RC $RESP => $SEARCH not found$NORM" 1>&2
        return $RC
    fi
...
state2col()
{
    SCOL=""
    local STA="$1"
    if test "$STA" == "ACTIVE" -o "$STA" == "active" -o "$STA" == "UP"; then
        SCOL="$GREEN"
    elif test "$STA" == "BUILD" -o "${STA:0:7}" == "PENDING" -o "$STA" == "creating" -o "$STA" == "downloading" -o "$STA" == "DOWN"; then
        SCOL="$YELLOW"
    elif test "${STA:0:5}" == "ERROR" -o "${STA:0:5}" == "error"; then
        SCOL="$RED"
    fi
}
...
createResources()
{
    ...
    eval local LIST=""  #FROSTY-!FIXME# eval local LIST=( \"\${${ORNM}S[@]}\" )
    eval local MLIST="" #FROSTY-!FIXME# eval local MLIST=( \"\${${MRNM}S[@]}\" )
    ...
    for no in $(seq 0 $(($QUANT - 1))); do
        ...
        if test "$1" = "cinder" && [[ $OS_AUTH_URL == *teutostack* ]]; then
            echo -en " ${RED}+5s${NORM} " 1>&2
            sleep 5
        fi
        if test $RC != 0; then
            echo -e "${YELLOW}ERROR: $RNM creation failed$NORM" 1>&2
            return 1
        fi
...
deleteResources()
{
    ...
    eval local LIST="" #FROSTY-!FIXME# eval local LIST=( \"\${${RNM}S[@]}\" )
    #echo $LIST
...
colstat()
{
    if test "$2" == "NONNULL" -a -n "$1" -a "$1" != "null"; then
        ...
        echo -e "${GREEN}${1:0:1}${NORM}"
        return 2
    elif test "${1:0:5}" == "error" -o "${1:0:5}" == "ERROR"; then
        echo -e "${RED}${1:0:1}${NORM}"
        return 1
    elif test -n "$1"; then
        echo "${1:0:1}"
    else
...
waitResources()
{
    ...
    #if test $TIMEOUTFACT -gt 1; then let TIMEOUT+=2; fi
    local STATI=()
    eval local RLIST="" #FROSTY-!FIXME# eval local RLIST=( \"\${${RNM}S[@]}\" )
    eval local SLIST="" #FROSTY-!FIXME# eval local SLIST=( \"\${${STIME}[@]}\" )
    local LAST=$((${#RLIST[@]} - 1))
    declare -i ctr=0
    declare -i WERR=0
    local TIRESP
    while test -n "${SLIST[*]}" -a $ctr -le 320; do
        ...
        for i in $(seq 0 $LAST); do
            local rsrc=${RLIST[$i]}
            if test -z "${SLIST[$i]}"; then
                STATSTR+=$(colstat "${STATI[$i]}" "$COMP1" "$COMP2")
                continue
            fi
            ...
            eval ${STATNM}+="( $TM )"
            if test $RC != 0; then
                echo -e "\n${YELLOW}ERROR: Querying $RNM $rsrc failed$NORM" 1>&2
                return 1
            fi
            STATI[$i]=$STAT
...
waitlistResources()
{
    ...
    local STATI=()
    eval local RLIST=""  #FROSTY-!FIXME# eval local RLIST=( \"\${${RNM}S[@]}\" )
    eval local RRLIST="" #FROSTY-!FIXME# eval RRLIST=( \"\${${RNM}S[@]}\" )
    eval local SLIST=""  #FROSTY-!FIXME# eval local SLIST=( \"\${${STIME}[@]}\" )
    local LAST=$((${#RLIST[@]} - 1))
...
waitdelResources()
{
    ...
    #if test $TIMEOUTFACT -gt 1; then let TIMEOUT+=2; fi
    eval local RLIST="" #FROSTY-!FIXME# eval local RLIST=( \"\${${RNM}S[@]}\" )
    eval local DLIST="" #FROSTY-!FIXME# eval local DLIST=( \"\${${DTIME}[@]}\" )
    local STATI=()
    local LAST=$((${#RLIST[@]} - 1))
    local STATI=()
    local TIRESP
    ...
    while test -n "${DLIST[*]}" -a $ctr -le 320; do
        ...
        for i in $(seq 0 $LAST); do
            local rsrc=${RLIST[$i]}
            if test -z "${DLIST[$i]}"; then
                STATSTR+=$(colstat "${STATI[$i]}" "XDELX" "")
                continue
            fi
...
createJHPorts()
{
    ...
    for i in $(seq 0 $((NOAZS - 1))); do
        ...
        NETSTATS+=($TM)
        if test $RC != 0; then
            echo -e "${YELLOW}ERROR: Failed setting allowed-adr-pair for port ${JHPORTS[$i]}$NORM" 1>&2
            return 1
        fi
    done
}
...
collectPorts_Old()
{
    ...
    for vm in $(seq 0 $(($NOVMS - 1))); do
        vmid=${VMS[$vm]}
        if test -z "$vmid"; then
            sendalarm 1 "nova list" "VM $vm not found" $NOVATIMEOUT
            continue
        fi
...
testLBs()
{
    ...
    if test "$TCP_LB" = "1"; then
        ...
        echo -n "(HTTP) "
        PROTO=HTTP
        SESSPERS="--session-persistence type=HTTP_COOKIE"
        URLPATH="--url-path /hostname"
    fi
    if test "$LB_PROVIDER" = "--provider ovn"; then
        echo -n "(${LB_PROVIDER##* }) "
        LB_ALGO="--lb-algorithm SOURCE_IP_PORT"
    fi
...
wait222()
{
    ...
    for JHNO in $(seq 0 $(($NOAZS - 1))); do
        ...
        if test $ctr -ge $MAXWAIT; then
            echo -e "${RED}JumpHost$JHNO (${FLOATS[$JHNO]}) not pingable${NORM}"
            let waiterr+=1
            perr=1
        fi
        # Now ssh
...
testlsandping()
{
    ...
    if test -z "$pport"; then
        ...
        ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "StrictHostKeyChecking=no" -o "ConnectTimeout=8" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 ls > /dev/null 2>&1 || {
            echo -n ".."
            sleep 8
            ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "StrictHostKeyChecking=no" -o "ConnectTimeout=16" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 ls > /dev/null 2>&1 || {
                echo -n "........"
                sleep 16
        ...
        if test -n "$BOOTALLATONCE"; then
            ...
            ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "StrictHostKeyChecking=no" -o "ConnectTimeout=8" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 grep api_monitor.sh.${RPRE} /tmp/testfile > /dev/null 2>&1 || {
                echo -n "o"
                sleep 4
            ...
            ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "StrictHostKeyChecking=no" -o "ConnectTimeout=8" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 grep api_monitor.sh.${RPRE}$4 /tmp/testfile > /dev/null 2>&1 || {
                echo -n "O"
                sleep 4
    ...
    PING=$(
        timeout 24 ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "ConnectTimeout=8" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 ping -c1 $PINGTARGET 2> /dev/null | tail -n2
        exit ${PIPESTATUS[0]}
    )
    RC=$?
    if test $RC = 0; then
    ...
    sleep 2
    PING=$(
        timeout 24 ssh -i $DATADIR/$1.pem $pport -o "PasswordAuthentication=no" -o "ConnectTimeout=8" -o "UserKnownHostsFile=~/.ssh/known_hosts.$RPRE" ${USER}@$2 ping -c1 $PINGTARGET2 2>&1 | tail -n2
        exit ${PIPESTATUS[0]}
    )
    RC=$?
...
testjhinet()
{
    ...
    for JHNO in $(seq 0 $(($NOAZS - 1))); do
        ...
        if test $ctr -ge 15; then
            echo -n "(ping timeout)"
            ERR="${ERR}ping ${FLOATS[$JHNO]}; "
        fi
        ...
        if test $R == 2; then
            RC=2
            ERR="${ERR}ssh JH$JHNO ls; "
        elif test $R == 1; then
            let CUMPINGERRORS+=1
            ERR="${ERR}ssh JH$JHNO ping $PINGTARGET || ping $PINGTARGET2; "
        fi
        if test $R != 0; then
            ostackcmd_tm NOVASTATS $NOVATIMEOUT nova show ${JHVMS[$JHNO]}
...
EOT
        ...
        echo
        if test -n "$LOGFILE"; then echo >> $LOGFILE; fi
        rm ${RPRE}wait
    fi
    return $RC
}
...
stats_old()
{
    ...
    eval local LIST="" #FROSTY-!FIXME# eval LIST=( \"\${${1}[@]}\" )
    ...
    if test $(($NO % 2)) = 1; then
        ...
        MED=$(math "%.${DIG}f" "(${SLIST[$MID]}+${SLIST[$(($MID - 1))]})/2")
    fi
    # PCT percentile
    NFQ=$(
        scale=3
        echo "(($NO-1)*$PCT)/100" | bc -l
    )
...
cleanup_new()
{
    ...
    delete2ndPorts
    deletePorts
    deleteJHPorts # not strictly needed, ports are del by VM del
    NOFITLERTAG=1
    waitdelLBs
...
cleanup()
{
    ...
    for sub in ${SUBNETS[*]}; do
        if ! echo "$sub" | grep '^[0-9a-f\-]\+' > /dev/null; then
            echo "#DEBUG: Skip port clean subnet $sub"
            continue
        fi
...
waitnetgone()
{
    ...
    KEYPAIRS=($(echo "$OSTACKCMD" | grep $RPRE | sed 's/^| *\([^ ]*\) *|.*$/\1/'))
    DKPS=(${KEYPAIRS[*]})
    deleteKeypairs
 ...
 VMs $REMVMS FIPs $REMFIPS JHVMs $REMJHVMS Keypairs $REMKPS Volumes $REMVOLS JHVols $REMJHVOLS" 0
    ...
    if test -n "$LOADBALANCER"; then
        for sub in ${SUBNETS[*]}; do
            if ! echo "$sub" | grep '^[0-9a-f\-]\+' > /dev/null; then
                echo "#DEBUG: Skip port cleanup in subnet $sub"
                continue
            fi
            PORTS=($(findres "octavia-lb" neutron port-list --fixed-ip subnet=$sub))
            echo "Cleaning octavia ports ${PORTS[*]} in subnet $sub ..."
    ...
    while test $to -lt 40; do
        SUBNETS=($(findres "" neutron subnet-list))
        NETS=($(findres "" neutron net-list))
        if test -z "${SUBNETS[*]}" -a -z "${NETS[*]}"; then
            echo "gone"
            unset NOFILTERTAG
            return
        fi
...
getPublicEP()
{
    ...
    if test -z "$EPS2" -a -n "$EPS"; then
        while read ln; do
            if echo "$ln" | grep '^[^:]*$' > /dev/null 2>&1; then
                REG=$ln
                continue
            fi
...
cleanprj()
{
    if test ${#OS_PROJECT_NAME} -le 5; then
        echo -e "${YELLOW}ERROR: Won't delete $OS_PROJECT_NAME$NORM" 1>&2
        return 1
    fi
...
parse_notification_addresses()
{
    ...
    for env_name in $(env | egrep API_MONITOR_ALARM_EMAIL\(_[0-9]+\)? | sed 's/^\([^=]*\)=.*/\1/'); do
        ALARM_EMAIL_ADDRESSES=("${ALARM_EMAIL_ADDRESSES[@]}" ${!env_name})
    done

    for env_name in $(env | egrep API_MONITOR_NOTE_EMAIL\(_[0-9]+\)? | sed 's/^\([^=]*\)=.*/\1/'); do
        NOTE_EMAIL_ADDRESSES=("${NOTE_EMAIL_ADDRESSES[@]}" ${!env_name})
    done

    for env_name in $(env | egrep API_MONITOR_ALARM_MOBILE_NUMBER\(_[0-9]+\)? | sed 's/^\([^=]*\)=.*/\1/'); do
        ALARM_MOBILE_NUMBERS=("${ALARM_MOBILE_NUMBERS[@]}" ${!env_name})
    done

    for env_name in $(env | egrep API_MONITOR_NOTE_MOBILE_NUMBER\(_[0-9]+\)? | sed 's/^\([^=]*\)=.*/\1/'); do
...
while test $loop != $MAXITER -a -z "$INTERRUPTED"; do
    ...
    if test "$1" = "CLEANUP"; then
        ...
        if test -n "$2"; then
            RPRE=$2
            if test ${RPRE%_} == ${RPRE}; then RPRE=${RPRE}_; fi
        fi
        ...
        while test $loop != $MAXITER -a -z "$INTERRUPTED"; do
            ...
            if test "$(($loop % 10))" == 0; then collectRes; else
                echo " Reuse known resources ..."
                sleep 2
            fi
            if test -z "${VMS[*]}"; then
                echo "No VMs found"
            ...
            let SUCCRUNS+=1
            if test $SUCCWAIT -ge 0; then sleep $SUCCWAIT; else
        ...
        if test -z "$JHIMGID" -o "$JHIMGID" == "0"; then
            sendalarm 1 "No JH image $JHIMG found, aborting." "" $GLANCETIMEOUT
            exit 1
        fi
        IMGID=$(ostackcmd_search "$IMG" $GLANCETIMEOUT glance image-list $IMGFILT | awk '{ print $2; }')
        if test -z "$IMGID" -o "$IMG" == "0"; then
            sendalarm 1 "No image $IMG found, aborting." "" $GLANCETIMEOUT
            exit 1
        fi
        let APICALLS+=2
        ...
        if test $? != 0; then
            let APIERRORS+=1
            sendalarm 1 "glance image-show failed" "" $GLANCETIMEOUT
            errwait $ERRWAIT
            let loop+=1
            continue
        else
    ...
    CTIME=$(date +%H:%M:%S)
    if test -n "$FULLCONN"; then
        CONNTXT="$CUMCONNERRORS Conn ERRORS"
        CONNST="|$CUMCONNERRORS"
    else
        CONNTXT=""
        CONNST=""
    fi
    if test -n "$LOADBALANCER"; then
        LBTXT="$CUMLBERRORS LB ERRORS"
        LBST="|$CUMLBERRORS"
    else
        LBTXT=""
        LBST=""
